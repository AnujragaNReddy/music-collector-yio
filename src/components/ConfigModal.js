...
export default function ConfigModal({ open, config, onClose, onSave }) {
  ...
  const [moveDestination, setMoveDestination] = useState(config.moveDestination);
  ...
  return (
    <div className="config-modal-overlay" onClick={onClose}>
      <form className="config-modal" onClick={(e) => e.stopPropagation()} onSubmit={handleSave}>
        ...
        <label className="config-field">
          ...
          <p className="config-hint">
            Direct file links, one per line — used by the Fetch button. This downloads exactly the files
            you list; it doesn't crawl a page for links.
          </p>
        </label>
        ...
        <label className="config-field">
          ...
          />
          <p className="config-hint">
            Where Fetch saves files, relative to the backend's Downloads folder. An absolute path (e.g.{' '}
            <code>D:\Music</code>) only works if the backend was started locally with{' '}
        ...
        <label className="config-field">
          ...
          />
          <p className="config-hint">
            Where selected files go when you use Copy/Move on the folder view below.
          </p>
        </label>

        <div className="config-modal-actions">
          <button type="button" className="config-cancel-btn" onClick={onClose}>Cancel</button>
          <button type="submit" className="config-save-btn">Save</button>
        </div>
      </form>
    </div>
  );
